...
$auth_users = array(
    'admin' => '$2y$10$RbX88BT.b2.UEoVFQ/tAPujjZUBS.j7UTkOTXQlx55RGzhHI9qJ/6', //admin@123
    'user' => '$2y$10$RbX88BT.b2.UEoVFQ/tAPujjZUBS.j7UTkOTXQlx55RGzhHI9qJ/6' //12345
);
...
if (defined('FM_EMBED')) {
    ...
    // FIX: Ensure session directory exists and is writable
    $session_save_path = ini_get('session.save_path');
    if (empty($session_save_path)) {
        $session_save_path = session_save_path();
    }
    if (empty($session_save_path) || $session_save_path === '0') {
        $session_save_path = sys_get_temp_dir();
    }
    
    // Check if session path is writable
    if (!is_writable($session_save_path)) {
        // Create a sessions directory in the current folder as fallback
        $fallback_path = __DIR__ . '/sessions';
        if (!is_dir($fallback_path)) {
            @mkdir($fallback_path, 0755, true);
        }
